...
from transformer_layer import TransformerEncoderLayer, _get_activation_fn
...
class TransformerModel(nn.Module):
    ...
    * Optional global attention tokens: prepended learnable embeddings that
    ...
    def __init__(self,
                encoder, ninp: int, nhead: int, nhid: int, nlayers: int,
                 dropout: float = 0.0,
                   style_encoder=None, 
                 y_encoder=None,
                 pos_encoder=None, 
        ...
        self.internal_feature_encoder = nn.Linear(2560, 256)
    ...
    def _forward(self, 
        ...
        #print(style_src.shape, x_src.shape, y_src.shape, global_internals.shape)
        # Feature embedding
        x_src = self.encoder(x_src)
        

        global_internals = global_internals.T.unsqueeze(0)
        global_internals = self.internal_feature_encoder(global_internals)
        ...
        src_mask = single_eval_pos + len(style_src) + global_internal_pos
