...
from dataset_loader.synthetic_data_generator_in import DataGenerator
import hydra
...
from trainer.trainer_global import make_model_od
...
def compute_internal_features(data):
    """
...
def main(cfg: DictConfig):
    ...
    model_path = '/home/xding2/PIQL/pretrained_ckpts/2layer_silver'
    ...
    for ckpt, filename in ckpt_infos:
        ...
        for dataset_list in [datagenerator.dataset_list_gmm]:
            ...
            for dataset in dataset_list:
                ...
                model_name = f'silver_{ckpt}'  # 'fomo'  #'fomoorigin2'  #'gmm'  #'fomoorigin2' #'fomo'
                save_path = f'results/gaussian_in/2layer_silver/{model_name}/{dataset}'
                print(save_path)
                ...
                def add_feature_transform_with_prior(x,y, mean, variance, anomaly_variance):
                    if preprocess_transform is None:
                        ...
                        if feature_dim < max_feature_dim:
                            ...
                            x = FT.feature_padding_torch(x=x, num_feature=x.shape[1])
                            x = x.numpy()
                        #updated: use two additional features!  
                        p_out = float((y == 1).mean())
                        p_in = 1 - p_out
                        global_var_all = p_in * variance + p_out * anomaly_variance
                        internal_features = np.concatenate([mean, global_var_all],axis=0)
                        x = np.concatenate([internal_features, x], axis=0)
                        y = np.concatenate([np.array([0]*internal_features.shape[0]),y],axis=0)
                ...
                def make_train_test(data):
                    ...
                    train_and_test,y, internal_features = add_feature_transform_with_prior(
                                                                    x=np.concatenate([train_x, x_test], axis=0),
                                                                    y=np.concatenate([y_train, y_test],axis=0),
                                                                    mean = global_mean, 
                                                                    variance = global_anomaly_variance,
                                                                    anomaly_variance = global_anomaly_variance)
                    eval_pos = len(train_x)
                    train_x, x_test = train_and_test[:eval_pos+internal_features, :], train_and_test[eval_pos+internal_features:, :]
                    y_train, y_test = y[:eval_pos+internal_features], y[eval_pos+internal_features:]
...
#CUDA_VISIBLE_DEVICES=1 python eval_gmm_silver_gaussian_in.py train.apply_linear_transform=False train.gen_one_train_one=True prior.mixture.max_feature_dim=100 prior.mixture.max_model_dim=100 train.seed=0 train.num_R=500 test.seed=0 test.preprocess_transform=null
