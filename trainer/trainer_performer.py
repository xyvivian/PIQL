...
from model_meta.transformer import TransformerModel
...
from model_meta import positional_encodings
...
class ZeroShotOD(pl.LightningModule):
    def __init__(self, 
        ...
        if continue_model:
            ...
            #to do: change the hard-coded one to automate
    ...
    def forward(self, full_data):
        data = (full_data.style.to(self.device) if full_data.style is not None else None, full_data.x.to(self.device),
            ...
            full_data.internal_xs.to(self.device) if full_data.internal_xs is not None else None)
