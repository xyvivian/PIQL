...
from trainer.trainer_mix_baseline import ZeroShotOD

torch.set_num_threads(24)
try:
    torch.set_num_interop_threads(24)
except RuntimeError as e:
    print(f"[threading] skip set_num_interop_threads(24): {e}")
...
def main(cfg: DictConfig) -> None:
    ...
    config_details = f'meta_baseline_multiprior_{seq_len}.feat{max_feature_dim}.R{num_R}.LT{apply_linear_transform}.gen1tr1{gen_one_train_one}.reuse{reuse_data_every_n}.E{epochs}.step{steps_per_epoch}.bs{batch_size}.lr{lr}.emb{emsize}.hdim{nhid}.nhead{nhead}.nlayer{nlayer}.ndevice{num_device}.T0{T0}_{current_time}'
...
#CUDA_VISIBLE_DEVICES=4,5,1,2 python pretrain_multi_baseline.py train.apply_linear_transform=False train.gen_one_train_one=True  train.seed=0 train.num_R=500 train.epochs=1500