...
def main(cfg: DictConfig):
    ...
    model_path = '/home/xding2/PIQL/pretrained_ckpts/2layer_bronze'
    ...
    for ckpt, filename in ckpt_infos:
        ...
        for dataset_list in [datagenerator.dataset_list_gmm]:
            ...
            for dataset in dataset_list:
                ...
                model_name = f'bronze_{ckpt}'  # 'fomo'  #'fomoorigin2'  #'gmm'  #'fomoorigin2' #'fomo'
                save_path = f'results/gaussian_in/2layer_bronze/{model_name}/{dataset}'
                print(save_path)
                ...
                def add_feature_transform_with_prior(x,y):
                    if preprocess_transform is None:
                        ...
                        internal_features = compute_internal_features(torch.from_numpy(x))
                        num_features = x.shape[1]
                        if feature_dim < max_feature_dim:
                            x = torch.from_numpy(x)
                            x = FT.feature_padding_torch(x=x, num_feature=x.shape[1])
                            x = x.numpy()
                            internal_features = torch.cat([internal_features, torch.zeros(internal_features.shape[0], 
                                                                                        100 - num_features, device=internal_features.device)], dim=-1)
                        internal_features = internal_features.detach().cpu().numpy()
                ...
                def make_train_test(data):
                    ...
                    train_and_test,y, internal_features = add_feature_transform_with_prior(x=np.concatenate([train_x, x_test], axis=0),
