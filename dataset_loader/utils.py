...
def get_batch_to_dataloader(get_batch_method_):
    ...
    class DL(PriorDataLoader):
        ...
        def get_test_batch(self, **kwargs): # does not increase epoch_count
            print('test batch is called...')
            return self.gbm(**self.get_batch_kwargs, epoch=self.epoch_count, model=self.model if hasattr(self, 'model') else None, num_step=0, **kwargs)

        def __iter__(self):
            assert hasattr(self, 'model'), "Please assign prior with `dl.model= ...` before training."
            self.epoch_count += 1
            return iter(self.gbm(**self.get_batch_kwargs, epoch=self.epoch_count - 1, model=self.model, num_step=i) for i in range(self.num_steps))
...
class PowerActivation(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        exp = torch.nn.Parameter((shared_exp * self.shared_exp_strength + shared_exp * torch.randn(x.shape[-1]) * (
                    1 - self.shared_exp_strength)) * 2 + 0.5).to(x.device)
