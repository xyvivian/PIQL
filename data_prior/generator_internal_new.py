...
class PriorTrainDataGenerator:  # generate synthetic data for training
    ...
    def get_batch_all_models(self, 
        ...
        def make_x_y_with_stored_data_internal(train_test_in, test_la, weight_variable=None):
            ...
            x = torch.cat([inliners, test_x], dim=0)  # (num_inliners+num_test_x, dim)
            internal_features = self.compute_internal_features(x)
        ...
        for data in list_of_data:
            ...
            elif internal_choice == 'internal_plus':
                x, y, prepend_dim = make_x_y_with_stored_data_internal_100(train_test_in=inliners, test_la=la, weight_variable=model_name)
