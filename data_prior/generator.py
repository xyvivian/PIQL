...
class PriorTrainDataGenerator:  # generate synthetic data for training
    ...
    def compute_internal_features(self,data):
        ...
        Returns:
            torch.Tensor with shape (4, dim), where rows are
        ...
        skewness = (centered ** 3).mean(dim=0) / (std ** 3)
        # Compute kurtosis (fourth standardized moment - 3)
        kurtosis_val = (centered ** 4).mean(dim=0) / (std ** 4) - 3
        # Stack per-dimension statistics into a (4, dim) tensor
        stats = torch.stack([mean, variance, skewness, kurtosis_val], dim=0)
        return stats
    ...
    def get_batch_all_models(self, 
        ...
        def make_x_y_with_stored_data(train_test_in, test_la, weight_variable=None):
        ...
        def make_x_y_with_stored_data_global(train_test_in, test_la, weight_variable=None):
            ...
            feature_dim = x.shape[-1]
            if feature_dim < self.max_feature_dim:
                x = self.FT.feature_padding_torch(x=x, num_feature=feature_dim, internal_features = None)
        ...
        def make_x_y_with_stored_data_internal(train_test_in, test_la, weight_variable=None):
            train_test_in = train_test_in[torch.randperm(train_test_in.shape[0])]
            test_la = test_la[torch.randperm(test_la.shape[0])]
            ...
            x = torch.cat([inliners, test_x], dim=0)  # (num_inliners+num_test_x, dim)
            internal_features = self.compute_internal_features(inliners)
        ...
        def make_x_y_with_stored_data_individual(train_test_in, test_la, weight_variable=None):
            inlier_mean, inlier_cov, outlier_mean, outlier_cov = weight_variable[2]
            train_order = torch.randperm(train_test_in.shape[0])
            test_order = torch.randperm(test_la.shape[0])
            train_test_in = train_test_in[train_order]
            inlier_mean = inlier_mean[train_order]
            inlier_cov = inlier_cov[train_order]
            test_la = test_la[test_order]
            outlier_mean = outlier_mean[test_order]
            oulier_cov = outlier_cov[test_order]

            inliners = train_test_in[:num_inliners]
            inliners_mean = inlier_mean[:num_inliners]
            inliners_cov = inlier_cov[:num_inliners]

            test_inliner = train_test_in[num_inliners:]
            test_inliner_mean = inlier_mean[num_inliners:]
            test_inliner_cov = inlier_cov[num_inliners:]
            
            test_la = test_la[:num_test_x]
            test_la_mean = outlier_mean[:num_test_x]
            test_la_cov = outlier_cov[:num_test_x]

            test_x = torch.cat([test_inliner, test_la], dim=0)
            test_mean = torch.cat([test_inliner_mean,test_la_mean],dim=0)
            test_cov = torch.cat([test_inliner_cov, test_la_cov], dim=0)
            test_y = torch.tensor([0] * num_test_x + [1] * num_test_x)
            ...
            test_mean = test_mean[sample_indices]
            test_cov = test_cov[sample_indices]

            x = torch.cat([inliners, test_x], dim=0)  # (num_inliners+num_test_x, dim)
            inliner_internal_features = torch.cat([inliners_mean, inliners_cov],dim=1)
            test_internal_features = torch.cat([test_mean, test_cov],dim=1)
            internal_features = torch.cat([inliner_internal_features,test_internal_features],dim=0)
            internal_feature_dim = internal_features.shape[1]
            x = torch.cat([internal_features, x], dim=1)  
            y = torch.cat([torch.tensor([ignore_index] * num_inliners), test_y], dim=0)

            feature_dim = x.shape[-1]
            if feature_dim < self.max_feature_dim + internal_feature_dim:
                x = self.FT.feature_padding_torch(x=x, num_feature=feature_dim, internal_features = internal_features)
            return x, y, 0
        ...
        for data in list_of_data:
            ...
            if internal_choice == 'internal':
                x, y, prepend_dim = make_x_y_with_stored_data_internal(train_test_in=inliners, test_la=la, weight_variable=model_name)
            elif internal_choice == 'global':
                x, y, prepend_dim = make_x_y_with_stored_data_global(train_test_in=inliners, test_la=la, weight_variable=model_name)
            elif internal_choice == 'individual':
                x,y, prepend_dim = make_x_y_with_stored_data_individual(train_test_in=inliners, test_la=la, weight_variable=model_name)
            elif internal_choice == 'baseline':
                x,y, prepend_dim = make_x_y_with_stored_data(train_test_in=inliners, test_la=la, weight_variable=None)
            else:
