...
class PriorTrainDataGenerator:  # generate synthetic data for training
    ...
    def generate_from_mixture(self, model,return_params=True): 
        ...
        num_LA = self.seq_len 
        out = model.draw_batched_data(num_inliers, num_LA, return_params=return_params)

        # Compatible with both old data_prior.GMM and training_embedder.gmm_test APIs.
        if not return_params:
            inliers, LA = out
            return inliers, LA

        if isinstance(out, tuple) and len(out) == 6:
            inliers, LA, inliers_mean, inlier_covariance, outlier_means, outlier_covariances = out
            return inliers, LA, inliers_mean, inlier_covariance, outlier_means, outlier_covariances

        # gmm_test currently returns only (inliers, local_anomalies) even with return_params=True.
        inliers, LA = out
        inliers_mean = torch.zeros_like(inliers)
        inlier_covariance = torch.zeros_like(inliers)
        outlier_means = torch.zeros_like(LA)
        outlier_covariances = torch.zeros_like(LA)

        if hasattr(model, "draw_samples"):
            try:
                _, inliers_mean, inlier_covariance, _ = model.draw_samples(num_inliers, return_params=True)
            except Exception:
                pass
        if hasattr(model, "draw_inflated_samples"):
            try:
                _, outlier_means, outlier_covariances, _ = model.draw_inflated_samples(num_LA, return_params=True)
            except Exception:
                pass
    ...
    def process_one_dataset(epoch_id,
        ...
        if model_name == "gmm":
            ...
            weight_variable = [model_mean,
                               ...
                               inflated_model_var]
            del model
            return inliers, LA, sub_dims, (model_name, description)
        else:
    ...
    def get_batch_all_models(self, 
        ...
        def make_x_y_with_stored_data_global(train_test_in, test_la):
        ...
        for data in list_of_data:
            ...
            if internal_choice == 'global':
                x, y = make_x_y_with_stored_data_global(train_test_in=inliners, test_la=la)
        ...
        return Batch(x=xs.transpose(0, 1), y=None, internal_xs= internal_xs, target_y=ys.transpose(0, 1),model_names = model_names, single_eval_pos=single_eval_pos)